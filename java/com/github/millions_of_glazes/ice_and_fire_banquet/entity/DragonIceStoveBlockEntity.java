...
public class DragonIceStoveBlockEntity extends SyncedBlockEntity {
    ...
    // 构造器中使用你自己的方块实体类型
    public DragonIceStoveBlockEntity(BlockPos pos, BlockState state) {
        super(ModEntities.DRAGON_ICE_STOVE.get(), pos, state);
    }
    ...
    // 核心烹饪逻辑（服务端）
    public static void cookingTick(Level level, BlockPos pos, BlockState state, DragonIceStoveBlockEntity stove) {
    ...
    public static void animationTick(Level level, BlockPos pos, BlockState state, DragonIceStoveBlockEntity stove) {
        for (int i = 0; i < stove.inventory.getSlots(); ++i) {
